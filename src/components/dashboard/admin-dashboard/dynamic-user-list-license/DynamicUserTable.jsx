...
export default function DynamicUserTable({ userData }) {
  ...
  const handleSuspendUser = useCallback(
    async (id) => {
      const url = `${process.env.NEXT_PUBLIC_BASE_URL}/api/license/change-status/${id}`;
      const response = await UPDATE_DATA_URL_ID(url, token);
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredUsers = filteredUsers.filter((user) =>
        user.service_name.toLowerCase().includes(filterValue.toLowerCase())
      );
    }
    if (
      statusFilter !== "all" &&
  ...
  }, [filterValue, onSearchChange, onRowsPerPageChange, userData.length]);
