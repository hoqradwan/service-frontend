...
const FreepikLinkGenerate = ({ dailyDownloadLimit, dailyDownload }) => {
  ...
  const onSubmit = (data) => {
    const url = data?.url;
    const content = url?.trim()?.split("/")[3];

    if (content === "icon") {
      setContentUrl(url);
      const iconOptions = [
        { label: "PNG (Static)", value: "png" },
        { label: "SVG (Static)", value: "svg" },
      ];

      setTypeOptions(iconOptions);
      setOpenTypesModal(!openTypesModal);
    } else if (content === "animated-icon") {
      setContentUrl(url);
      const iconOptions = [
        { label: "PNG (Static)", value: "png" },
        { label: "SVG (Static)", value: "svg" },
        { label: "JSON (Animated)", value: "json" },
        { label: "AEP (Animated)", value: "aep" },
        { label: "MP4 (Animated)", value: "mp4" },
      ...
      const videoOptions = [
        ...
        { label: "4K (Video)", value: "4K" },
        { label: "1080p (Video)", value: "1080p" },
        { label: "720p (Video)", value: "720p" },
      ];
    ...
    >
      <div className="w-full">
        <input
          ...
          placeholder="Enter your freepik Content URL"
          type="text"
...
export default FreepikLinkGenerate;
