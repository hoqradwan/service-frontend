...
import TypesModal from "../../../Shared/TypesModal/TypesModal";

const MotionArrayLinkGenerate = ({ dailyDownloadLimit, dailyDownload }) => {
  ...
  const onSubmit = (data) => {
    // console.log(data, "data from motion array")
    const url = data?.url;
    const content = url?.split("/")[3];
    if (content === "stock-video") {
      setContentUrl(url);
      const videoOptions = [
  ...
  const downloadRequest = async (data) => {
    ...
    try {
      const generateLinkResponse = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/download/motion-array`,
        {
          method: "POST",
          headers: {
    ...
    >
      <div className="w-full">
        <input
          ...
          placeholder="Enter your motion array Content URL"
          type="text"
...
export default MotionArrayLinkGenerate;
