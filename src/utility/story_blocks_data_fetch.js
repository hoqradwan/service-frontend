...
export const fetchLicenseData = async (
  ...
  try {
    ...
    // Filter for Story-blocks service
    const data = result?.data?.data;
    const storyBlocksLicense = data.find(
      (item) => item.serviceName === "Story-blocks"
    );
...
export const fetchDailyDownloadData = async (setDailyDownload) => {
  try {
    ...
    const token = await Cookies.get("session");
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/download/user-daily-story-blocks-download/${userInfo.id}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
...
export const fetchTotalDownloadData = async (setTotalDownload) => {
  try {
    ...
    const token = await Cookies.get("session");
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/download/user-total-story-blocks-download/${userInfo.id}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
...
export const fetchServiceStatusData = async (setServiceStatus) => {
  try {
    ...
    const response = await fetch(
      ...
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/user/service-status/story-blocks`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
